...
# Stage 2: Verifier — 원본 PDF 독립 재추출 후 대조
# Stage 3: Structural Checker — 파일명-내용 일치, frontmatter 무결성, 구조 검증
#
# Usage: ./scripts/verify-knowledge-db.sh [--stage 1|2|3|all] [--verbose] [--file <path>]
set -euo pipefail
...
stage1_extract() {
  ...
  local src_path="" src_title="" src_pages="" src_method=""
  ...
  verbose "law: $law_numbers"
  verbose "gosi: $gosi_numbers"
}
...
stage2_verify() {
  ...
  local src_pdf="${SRC_BASE}/${src_rel}"
  if [ ! -f "$src_pdf" ]; then
...
stage3_structure() {
  ...
  local has_source has_title has_path has_pages has_converted
...
if [ -n "$TARGET_FILE" ]; then
  ...
  find "$DST_BASE" -name "*.md" | sort > "$FILE_LIST"
fi
...
# Exit code
total_fail=$((ext_fail + ver_fail + str_fail))
