...
### Traditional Product Code (Axxxxx.xx Format)

**IMPORTANT**: Traditional MFDS product codes (format: `Axxxxx.xx`, e.g., `A09020.02`, `A45020.01`) CANNOT be generated by this skill. These codes are assigned by MFDS based on the official product classification table (「의료기기 품목 및 품목별 등급에 관한 규정」).

- Traditional codes require **lookup** from the official MFDS product classification database
- Do NOT fabricate or guess traditional codes — incorrect codes lead to wrong grade and pathway
- If a traditional code is needed, state: "Traditional product code requires MFDS database verification"
- The 7-digit digital medical device code system is SEPARATE from the traditional Axxxxx.xx system
