...
θ
 is greater than 0.4 (the shaded area) is 0.60. 
...
between observed outcomes and the parameter 
θ
. The likelihood function can 
be expressed in symbols by P(data |
θ
), which is the conditional probability of 
observing the data given a specific value of the parameter 
θ
, for each possible 
value of 
θ
...
θ
 conditional on the observed data, which can 
be denoted in symbols as P(
θ
| data).  Bayes’ theorem is used to update the prior 
distribution for 
θ, 
P(
θ
), via the likelihood, P(data|
θ
...
, P(
θ
|data).  The information about 
θ
 is summarized by this 
